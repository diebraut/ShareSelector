...
def main():
    ...
    ap.add_argument("--out", default="backtest_transaction_costs")
    ...
    # Aktuell beste Bruttovariante
    lookback = 20
    sell_thr = -0.15
    buy_thr = 0.02
    min_turnover = 1_000_000

    # Einfache Roundtrip-Kosten entsprechen ungefähr dem Doppelten:
    # 0.05 % je Trade -> 0.10 % Kauf+Verkauf
    # 0.10 % je Trade -> 0.20 % Roundtrip
    # 0.20 % je Trade -> 0.40 % Roundtrip
    # 0.30 % je Trade -> 0.60 % Roundtrip
    cost_levels = [0.0, 0.0005, 0.0010, 0.0020, 0.0030]

    summary_rows = []
    crash_rows = []

    print()
    print("Strategie: 20T / Sell -15% / Buy +2% / Liquiditaet >= 1 Mio.")
    print("Kosten werden auf JEDEN Kauf und JEDEN Verkauf angewendet.")

    for n, cost in enumerate(cost_levels, 1):
        print()
        print("=" * 72)
        print(
            f"[{n}/{len(cost_levels)}] Kosten je Transaktion: "
            f"{cost:.2%}  (ca. {2*cost:.2%} Roundtrip)"
        )
        ...
        tag = f"{cost*10000:.0f}bp"
        daily.to_csv(
            out_dir / f"daily_cost_{tag}.csv",
            encoding="utf-8-sig"
        )
    ...
    # Wirkung gegenüber kostenfreiem Backtest
    base_cagr = summary.iloc[0]["CAGR"]
    base_total = summary.iloc[0]["TotalReturn"]
    summary["CAGR_Loss_vs_ZeroCost"] = base_cagr - summary["CAGR"]
    summary["TotalReturn_Loss_vs_ZeroCost"] = base_total - summary["TotalReturn"]

    summary.to_csv(
        out_dir / "transaction_cost_summary.csv",
        index=False,
        encoding="utf-8-sig"
    )

    pd.DataFrame(crash_rows).to_csv(
        out_dir / "transaction_cost_crash_metrics.csv",
        index=False,
        encoding="utf-8-sig"
    )
