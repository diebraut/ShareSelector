...
def main():
    ...
    ap.add_argument("--out", default="backtest_final_optimization")
    ap.add_argument("--min-price", type=float, default=0.50)
    ap.add_argument("--transaction-cost", type=float, default=0.0010)
    args = ap.parse_args()
    ...
    momentum_windows = [15, 20, 25]
    sell_thresholds = [-0.125, -0.15, -0.175]
    buy_thresholds = [0.01, 0.02, 0.03]
    ...
    print()
    print("Finale Parameteroptimierung")
    print("=" * 72)
    print(f"Liquiditaet >= {min_turnover:,.0f}")
    print(f"Mindestkurs >= {args.min_price}")
    print(f"Kosten je Kauf/Verkauf = {args.transaction_cost:.2%}")
    ...
    for i, (lookback, sell_thr, buy_thr) in enumerate(variants, 1):
        name = (
            f"mom{lookback}_"
            f"sell{str(abs(sell_thr)*100).replace('.','_')}_"
            f"buy{str(buy_thr*100).replace('.','_')}"
        )
        ...
        summary_rows.append({
            ...
            "TransactionCost": args.transaction_cost,
            **met,
        })
    ...
    summary.to_csv(
        out_dir / "final_optimization_summary.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    summary.sort_values(
        ...
        out_dir / "final_ranked_by_cagr.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    summary.sort_values(
        ...
        out_dir / "final_ranked_by_sharpe.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    summary.sort_values(
        ...
        out_dir / "final_ranked_by_return_drawdown.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    crashes.to_csv(
        out_dir / "final_crash_metrics.csv",
        index=False,
        encoding="utf-8-sig"
    )

    # Combined table: one row per variant with crash metrics beside summary metrics.
