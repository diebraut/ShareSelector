    ...
    const QStringList statements = {
        ...
        QStringLiteral(R"SQL(
            CREATE TABLE IF NOT EXISTS "Depots" (
                "DepotId" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                "Name" TEXT NOT NULL UNIQUE,
                "StartInvest" DATE,
                "StartInvestAmount" NUMERIC(20, 2) NOT NULL DEFAULT 0,
        ...
        QStringLiteral(R"SQL(
            ...
            VALUES (1, 'Default', 110000, 'EUR', TRUE)
            ON CONFLICT ("DepotId") DO NOTHING
        )SQL"),
        QStringLiteral(R"SQL(
            UPDATE "Depots"
            SET "StartInvestAmount" = 110000,
        ...
        QStringLiteral(R"SQL(
            SELECT setval(
                pg_get_serial_sequence('"Depots"', 'DepotId'),
                GREATEST((SELECT COALESCE(MAX("DepotId"), 1) FROM "Depots"), 1),
                TRUE
            )
        )SQL"),
        QStringLiteral(R"SQL(
            CREATE TABLE IF NOT EXISTS "BoughtStocks" (
                "DepotId" INTEGER NOT NULL DEFAULT 1,
        ...
        QStringLiteral(R"SQL(
            UPDATE "Depots" d
            SET "StartInvest" = first_buy.first_buy_date,
                "UpdatedAt" = CURRENT_TIMESTAMP
            FROM (
                SELECT "DepotId", MIN("BuyDate") AS first_buy_date
                FROM "BoughtStocks"
                WHERE "SellDate" IS NULL
                  AND COALESCE("Status", 0) <> 10
                GROUP BY "DepotId"
            ) first_buy
            WHERE d."DepotId" = first_buy."DepotId"
              AND d."StartInvest" IS DISTINCT FROM first_buy.first_buy_date
        )SQL"),
        QStringLiteral(R"SQL(
            CREATE INDEX IF NOT EXISTS "BoughtStocks_DepotId_idx"
                ON "BoughtStocks" ("DepotId")
        )SQL"),
        QStringLiteral(R"SQL(
            DO $$
            BEGIN
                IF NOT EXISTS (
                    SELECT 1
                    FROM pg_constraint
                    WHERE conname = 'BoughtStocks_DepotId_fkey'
                ) THEN
                    ALTER TABLE "BoughtStocks"
                        ADD CONSTRAINT "BoughtStocks_DepotId_fkey"
                        FOREIGN KEY ("DepotId")
        ...
        QStringLiteral(R"SQL(
            ...
            BEGIN
                ...
                FROM pg_constraint
                WHERE conrelid = '"BoughtStocks"'::regclass
                  AND contype = 'p';
                ...
                IF current_pkey IS NOT NULL THEN
                    IF NOT EXISTS (
                        ...
                        JOIN pg_attribute a1
                          ON a1.attrelid = i.indrelid
                         AND a1.attnum = i.indkey[0]
                        JOIN pg_attribute a2
                          ON a2.attrelid = i.indrelid
                         AND a2.attnum = i.indkey[1]
                        WHERE i.indrelid = '"BoughtStocks"'::regclass
                          AND i.indisprimary
                          AND i.indnkeyatts = 2
                          AND a1.attname = 'DepotId'
                          AND a2.attname = 'Symbol'
                    ) THEN
                        EXECUTE format('ALTER TABLE "BoughtStocks" DROP CONSTRAINT %I', current_pkey);
                        ALTER TABLE "BoughtStocks"
                            ADD CONSTRAINT "BoughtStocks_pkey"
                            PRIMARY KEY ("DepotId", "Symbol");
                    END IF;
                END IF;
            END $$
        )SQL"),
